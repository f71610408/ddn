  ...
  node.api
    ...
    .end((err, { body }) => {
      ...
      if (body.success === true && body.account !== null) {
        ...
        debug(
          `Data sent: secret: ${Account1.password} , secondSecret: ${Account1.secondPassword}`
        );
...
describe("PUT /dapps", () => {
  ...
  it("Category is number, Using invalid Category, Should fail", async done => {
    // const time = Date.now();
    // debug("await start on new block async in category is number");
    await node.onNewBlockAsync();
    // debug(
    //   "await end on new block async in category is number",
    //   Date.now() - time
    // );
    node.api
      .put("/dapps")
  ...
  it("Using very long description. Should fail", done => {
    node.api
      ...
      .send({
        ...
        name: node.randomDelegateName(),
        description:
          "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient c",
        link: dappLink,
        icon: dappDemo.icon
      })
      .expect("Content-Type", /json/)
  ...
  it("Using very long tag. Should fail", done => {
    node.api
      ...
      .send({
        ...
        description: "A dapp that should not be added",
        tags:
          "develop,rice,voiceless,zonked,crooked,consist,price,extend,sail,treat,pie,massive,fail,maid,summer,verdant,visitor,bushes,abrupt,beg,black-and-white,flight,twist",
        link: dappLink,
        icon: dappDemo.icon
      })
      .expect("Content-Type", /json/)
  ...
  it("Using valid Link. Should be ok", done => {
    ...
    const time = Date.now();
    node.onNewBlock(() => {
      debug("Using valid Link. Should be ok", Date.now() - time);
      DappName = node.randomDelegateName();
      node.api
        .put("/dapps")
        .set("Accept", "application/json")
...
describe("GET /dapps", () => {
  ...
  it("Using invalid parameter type (link). Should fail", done => {
    ...
    node.api
      .get(
        `/dapps?category=${category}&name=${name}&type=${type}&link=${link}&icon=${icon}`
      )
      .expect("Content-Type", /json/)
      .expect(200)
      .end((_err, { body }) => {
      ...
      .end((_err, { body }) => {
        ...
        if (dapps[0] !== null) {
          for (let i = 0; i < dapps.length; i++) {
            if (dapps[i + 1] !== null) {
              node
                .expect(dapps[i].category)
                .to.be.at.most(dapps[i + 1].category);
            }
          }
        }
        done();
        ...
        if (dapps[0] !== null) {
          for (let i = 0; i < dapps.length; i++) {
            if (dapps[i + 1] !== null) {
              node
                .expect(dapps[i].category)
                .to.be.at.least(dapps[i + 1].category);
            }
          }
        }
        done();
  ...
  it("Using category. Should be ok", done => {
    ...
    node.api
      .get(`/dapps/category/${randomCategory}/all`)
      .expect("Content-Type", /json/)
      .expect(200)
      .end((_err, { body }) => {
        // debug(JSON.stringify(body));
        ...
        if (dapps.length > 0) {
          node
            .expect(dapps[0].category)
            .to.equal(node.DappCategory[randomCategory]);
        }
        done();
      });
  });
